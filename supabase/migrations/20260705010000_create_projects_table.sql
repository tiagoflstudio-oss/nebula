...
CREATE POLICY "Users can view projects if owner or admin/vip" ON projects
  FOR SELECT
  USING (
    auth.uid() = user_id 
    OR 
...
CREATE POLICY "Users can update projects if owner or admin" ON projects
  FOR UPDATE
  USING (
    auth.uid() = user_id
    OR
...
CREATE POLICY "Users can delete projects if owner or admin" ON projects
  FOR DELETE
  USING (
    auth.uid() = user_id
    OR
